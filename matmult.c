...
//clock_t begin, end;
  ...
  double time_spent, wcalcTime;
  double sum_wcalcTime=0, max_wcalcTime=0;
...
// Do some basic error checking
      ...
      for (i=1; i<=numworkers; i++)
      {
        ...
        time_spent = tdiff(begin, end);
		 ...
		 if(max_wcalcTime < wcalcTime)
		 {
		  max_wcalcTime = wcalcTime;
		 }
      }
	  ...
	  printf("sum of the wcalcTime for all worker ranks: %.8f sec\n", sum_wcalcTime);
      printf("total time (time spent): %.8f sec\n", time_spent);      
      printf ("\n");
   }
...
/**************************** worker task ************************************/
   if (taskid > MASTER)
   {
      ...
      // start of wcalcTime for worker rank
      begin = now();
      ...
      // end of wcalcTime for worker rank
      end = now();
      ...
      // sending worker rank wcalcTime to master
